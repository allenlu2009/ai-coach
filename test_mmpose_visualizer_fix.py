#!/usr/bin/env python3
"""
Test the fixed MMPose visualizer with raw inference_topdown results.
"""
...
def main():
    """Test the fixed MMPose visualizer."""
    ...
    try:
        ...
        analyzer = RTMPoseAnalyzer(use_3d=True, enable_detection=True, use_demo_visualizer=True)
        ...
        # Test the fixed MMPose visualizer
        print("\n🎨 Testing fixed MMPose visualizer...")
        
        # Test visualization directly using the raw results
        print("🖼️ Testing visualization with raw inference results...")
        vis_result = analyzer.mmpose_visualizer.process_and_visualize(
            frame=frame,
            pose_results=raw_pose_results,
            save_path="test_mmpose_visualizer_fix_output.jpg"
        )
        
        if vis_result is not None:
            print("✅ MMPose visualization successful!")
            print(f"   - Result shape: {vis_result.shape}")
            print("   - Saved to: test_mmpose_visualizer_fix_output.jpg")
            return True
        else:
